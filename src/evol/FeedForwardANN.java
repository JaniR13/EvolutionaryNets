...
public class FeedForwardANN {
    ...
    //number of layers (including input, output, hidden)
    public int layers;
     ...
     * @param logistic will a logistic activation function be used? (if not,
     * defaults to linear)
    ...
    public void setInputs(ArrayList<Double> newInputs) {
        ...
        for (int i = 1; i <= newInputs.size(); i++) {
            ArrayList <Double> nodeInput = new ArrayList<Double>();
            nodeInput.add(newInputs.get(i - 1));
            nodes.get(0).get(i).setInputs(nodeInput);
            nodes.get(0).get(i).setOutput(newInputs.get(i - 1));
        }
    }
